...
const baseUrl = 'https://www.googleapis.com/download/storage/v1/b/chromium-browser-snapshots/o/';
const getUrl = (target: Target) => resolve(
  baseUrl,
  `${target.prefix}%2F${revision}%2F${target.name}.zip?alt=media`,
);

const baseDirectory = join(process.cwd(), 'chromium');
const getArchivePath = (target: Target) => join(baseDirectory, `${target.name}.zip`);

const download = (from: string, to: string) => new Promise<void>(resolve => {
...
const unzip = (from: string, to: string) => new Promise((resolve, reject) => {
